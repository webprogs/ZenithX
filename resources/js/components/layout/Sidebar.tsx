...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#eaecef] transform transition-transform duration-300 ease-in-out',
          'lg:translate-x-0',
          isOpen ? 'translate-x-0' : '-translate-x-full'
        )}
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between h-16 px-6 border-b border-[#eaecef]">
          ...
          <div className="p-4 border-t border-[#eaecef]">
            <div className="flex items-center px-4 py-3 bg-[#f5f5f5] rounded-lg">
              <div className="w-10 h-10 bg-[#f0b90b] rounded-full flex items-center justify-center flex-shrink-0">
                <span className="text-[#1e2329] font-medium text-sm">
                  {user?.name?.charAt(0).toUpperCase()}
                </span>
              </div>
              <div className="ml-3 min-w-0">
                <p className="text-sm font-medium text-[#1e2329] truncate">{user?.name}</p>
